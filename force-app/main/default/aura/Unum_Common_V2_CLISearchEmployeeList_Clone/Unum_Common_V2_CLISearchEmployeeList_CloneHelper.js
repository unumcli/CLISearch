({
    ...
    saveComponentHealer : function (component, event, helper){
        if(component.get("v.CLISearchClaimLeaveListFlag")){
            var element = component.find("pdfForm").getElements()[0].innerHTML;
            var data = [];
            var obj = {};
            obj.sobjectType ="PDFStorage__c";
            obj.Content__c = element;
            data.push(obj);
            helper.savePDFHTML(component, event, helper,"saveHTML",data);
        }
    },
    savePDFHTML :function (component, event, helper,methodName,data){
        ...
        action.setCallback(this, function(response) {
            ...
            if(state =="SUCCESS"){
                ...
                if(result[0].Id){
                    ...
                    window.open('/apex/UNM_PDFMainPage?recordId='+recordId,'_self');  
                }
            }else{
