({
  ...
  /*  doInit : function(component,event,helper)
   ...
   //     var csvFile;
    ...
    {
        debugger
        var data = JSON.stringify(component.get("v.ClaimLeaveList"));
        //var element = document.querySelector(".pdfForm");
        var compEvent = component.getEvent("pdfEvent");
        compEvent.setParams({"type" : "PDF"});
        compEvent.fire();
        /*var url = '/apex/UNM_MainPage?data=test'; 
       ...
       // window.print();
    /*     var name = "sharma";
        var encodedString = btoa(name);
        var hello = window.btoa('Hello, world');
         var hiddenElement = document.createElement('a');
        hiddenElement.href = 'data:application/pdf;base64,' + encodeURI(hello);
        hiddenElement.target = '_blank';
        hiddenElement.download = 'unum'+'.pdf'; 
        hiddenElement.click();	*/
